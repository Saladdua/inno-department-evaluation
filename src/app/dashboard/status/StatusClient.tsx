...
export default function StatusClient({ periodLabel, periodStatus, stats, overall, canManageAll }: Props) {
    ...
    <div className="st-root">
      ...
      <div className="st-topbar">
        <div className="st-topbar-left">
          ...
          <span className={`st-period-status ${periodStatus === 'open' ? 'st-period-status--open' : ''}`}>
            {PERIOD_STATUS_LABEL[periodStatus] ?? periodStatus}
          </span>
        </div>
          ...
          <tbody>
            {stats.map(s => {
              ...
              return (
                <tr
                  key={s.id}
                  className={`st-tr ${s.isMyDept ? 'st-tr--mine' : ''}`}
                >
      ...
      <style>{`
        ...
        .st-prog-draft {
          position: absolute; top: 0; height: 100%;
          background: rgba(251,191,36,0.5); border-radius: 2px;
        ...
        .st-badge--in_progress {
          background: rgba(251,191,36,0.08); color: #fbbf24;
          border: 1px solid rgba(251,191,36,0.18);
        }
        .st-badge--not_started {
        ...
        .st-tag--more {
          background: rgba(179,0,0,0.08); color: rgba(179,0,0,0.7);
        ...
        /* ── Light mode ───────────────────────────────── */
        [data-theme="light"] .st-period-label { color: rgba(0,0,0,0.4); }
